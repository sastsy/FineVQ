...
from internvl.model.internvl_chat_stage2_lora_improve_eval import (InternVisionConfig,
                                          InternVisionModel,
                                          InternVLChatConfig,
                                          InternVLChatModel)
...
def save_and_evaluate(data_list, output_file='results_eval.csv', metrics_file='eval.txt'):
    ...
    df = pd.DataFrame(data_list, columns=['video_name', 'mos','pred_score'])
    # Save to CSV
...
def main():
    ...
    with torch.no_grad():
        for item2 in train_dataloader:
            ...
            data_list.append([video_name, mos_score, score1])
