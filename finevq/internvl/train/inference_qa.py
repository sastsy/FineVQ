...
class CustomArguments:
    ...
    use_pretrained_lora: bool = field(
        default=False,
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        question_txt = meta.get('question_txt')

        if video_name_txt and os.path.exists(video_name_txt):
            logger.info(f'Loading video list from {video_name_txt}')
            assert video_name_txt.endswith('txt'), f'video_name_txt must be txt, but got {video_name_txt}'
            with open(video_name_txt, 'r') as f:
                self.raw_data = f.readlines()
        else:
            logger.info(f'video_name_txt not provided or not found. Scanning {self.root} for videos.')
            self.raw_data = []
            video_extensions = ['.mp4', '.avi', '.mov', '.mkv', '.flv', '.wmv']
            for filename in os.listdir(self.root):
                if any(filename.lower().endswith(ext) for ext in video_extensions):
                    self.raw_data.append(filename)
            logger.info(f'Found {len(self.raw_data)} videos in {self.root}.')

        # Load questions from question.txt
        if question_txt and os.path.exists(question_txt):
            logger.info(f'Loading questions from {question_txt}')
            ...
            logger.warning(f'question_txt not provided or not found: {question_txt}')
            # Default question if file not found
            self.questions = ['How would you rate the overall quality of this video?']
    ...
    def multi_modal_get_item(self, data_item):
        ...
        mos = torch.tensor(float(data_item['mos']), dtype=torch.bfloat16)
    ...
    def multi_modal_multi_image_get_item(self, data_item):
        ...
        return ret
    def get_index(self, bound, fps, max_frame, first_idx=0, num_segments=8):
        if bound:
    ...
    def video_get_item(self, data_item):
        ...
        pixel_values = torch.stack(pixel_values)
        pixel_values2 = [transform(image) for image in image_list2]
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        while True:
            try:
                video_name = self.raw_data[i].strip()
                # Get question for this video (cycle through questions if more videos than questions)
                question = self.questions[i % len(self.questions)]
                # Ensure question starts with video placeholder
                ...
                if not isinstance(e, UnidentifiedImageError):
                    traceback.print_exc()
                video_name = self.raw_data[i].strip()
                data_path = os.path.join(self.root, video_name)
...
def build_datasets(
    ...
    datasets = []
    lengths = []
    print('data_args.meta_path',data_args.meta_path)
...
def save_and_evaluate(data_list, output_file='results_eval.csv', metrics_file='eval.txt'):
...
def main():
    ...
    if model_args.model_name_or_path is not None:
        ...
        if custom_args.use_pretrained_lora:
    ...
    data_list = []
    with torch.no_grad():
        for item2 in train_dataloader:
            # item = {k: v.to(model.device) for k, v in item2.items()}
            item = {}
           ...
           # Forward pass
            ...
            decoded_output = tokenizer.decode(output['logit'][-len(filtered_list) - 1:-1])
            print(decoded_output)

            print('answer',decoded_output)
            # Store the result for evaluation
            data_list.append([video_name, decoded_output])

    # Save results to CSV and calculate SRCC, PLCC, KRCC
    save_and_evaluate(data_list, output_file=custom_args.output_file, metrics_file=custom_args.metrics_file)
